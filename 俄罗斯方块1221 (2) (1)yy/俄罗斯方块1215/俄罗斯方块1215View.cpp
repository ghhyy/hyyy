	...
	int i;
	 ...
	 if(Shape-1==4)
	 {
		 ...
		 for(i=1;i<=3;i++)
		 {
			 ...
			 SJX[m_nJX].JX[i].m_y=SJX[m_nJX].JX[i].m_y+i*25;
		 }
	 }
     if(Shape-1==5)
	 {
	...
	for(i=0;i<4;i++)
		for(j=0;j<m_nJX;j++)
			for(k=0;k<4;k++)
			{
				...
				if(SJX[j].JX[k].m_y==SJX[m_nJX].JX[i].m_y&&SJX[j].JX[k].m_x==SJX[m_nJX].JX[i].m_x<0)
				{
					f_y=-2;
					TuXing[m_s]=0;
					b=SJX[m_nJX].JX[i].m_y-SJX[j].JX[k].m_y;
					for(j=0;j<4;j++)
						SJX[m_nJX].JX[j].m_y=SJX[m_nJX].JX[j].m_y-b;
					m_nJX++;
					m_s++;
